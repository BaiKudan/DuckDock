...
```
   ...
   外部变更单号、精确 Kubernetes context/Namespace、`kube-system` Namespace UID、当前认证 principal，
   以及本次执行允许使用的 Secret 名、CNI DaemonSet、三组探针 Namespace/Pod 和 HA drain zone、
...
```
   ...
   target-deployment/TLS/network/secrets/capacity/alerting/recovery/state-services/HA 九个带精确
...
```
   ...
   `target-cluster-identity.json` 及签名。随后仍由同一 Operations 身份采集 campaign 固定的
   Kubernetes RBAC allow/deny 轮廓：
...
```
   ...
   工具逐项运行固定的 `kubectl auth can-i`：只允许执行所需的读取、精确 Secret 读取、
   Deployment patch、Pod exec/eviction 和 node patch，并要求 Namespace/Node/Deployment 删除、
   Secret 广泛读取或修改、RBAC 变更/escalate/bind、身份 impersonate 等危险能力均返回拒绝。
   结果与变更单、集群身份和精确资源范围一起签名且不可覆盖。network phase 依赖身份和访问
   两对摘要；正式 network、secrets、HA CLI 在 phase permit 通过后、目标 I/O 或 mutation
   前会重新查询 UID/principal、重跑完整权限矩阵并逐字段核对运行参数。该矩阵不是 Kubernetes
   全部有效权限的穷举，也不验证外部工单系统的审批真实性；真实云 IAM、审计日志、工单审批
   与平台外管理员路径仍须发布机构独立控制。
   ...
   参数中的 secret 或个人数据，摘要由工具自行计算，调用方不能传入伪造摘要：

```bash
python backend/scripts/start_ga_execution_phase.py \
  --campaign /release-authority/duckdock-2.0.0-execution-campaign.json \
  --phase-id tls \
  --action-id CHG-2026-001/tls-probe \
  --action-description 'Run approved external TLS probe from the named vantage' \
  --operations-identity operations-release-authorizer@corp.example \
  --key /secure/keys/operations-release-authorizer
```

   工具重验原始 Security/Operations 双签、活动窗口、完整 phase projection 和所有依赖 phase
   的最终产物及摘要，然后只在 campaign 预分配路径写入不可覆盖的
   `duckdock-ga-execution-phase-start-v1` statement/signature。每个 phase 必须各运行一次；
   statement/signature 不能跨 phase 或 campaign 复用。closure 会把许可时间与 TLS/network
   ...
   `action-id` 必须以 campaign 的变更单号和 `/` 开头，例如 `CHG-2026-001/tls-probe`；错误
   或未绑定的工单前缀会在签署 phase permit 前失败。

   九个正式目标入口还会在开始部署、网络请求、轮换、负载、恢复或故障注入前重新验证该许可。
   ...
   外部执行期间不要等到第 102 份产物才发现早期错误。可随时运行非授权增量检查器：
...
```
   ...
   检查 102 份外部产物的 missing/regular-file/symlink/size/JSON/private-key marker，并验证
...
4. 运行 `python3 scripts/verify-production-baseline.py`，保留 JSON；然后用
   ...
   文件的 server-side dry-run。使用 `deploy-kubernetes-ha-target.py preflight` 重验同一
   execution campaign、Security/Operations 双签、release provenance 和目标访问签名，并把
   真实 kube-system UID/principal、三 zone、metrics、Secret/TLS 元数据、RWX
   StorageClass、Namespace 标签、批准 egress 和固定部署 RBAC 绑定到一小时内有效的
   可搬运回执；为 `target_deployment` 签署独立 phase permit 后，再用同一 campaign、
   action ID、精确 change request 和内容寻址确认串执行 `deploy`。执行器按 bootstrap → PVC Bound →
...
闭环工具按 campaign 预分配路径要求 102 份外部产物全部存在，拒绝 symlink、缺失、摘要
冲突和任何指向未计划路径的 `path+sha256`、`path+manifest_sha256`、allowed-signers 或
signature 引用；十份最终 wrapper 及部署回执的 `observed_at` 必须位于执行窗口内，九个风险 phase
...
execution closure 的 campaign/request/topology、102 份外部产物、至少 126 个闭包输入、294 条以上
...
签名 statement，因此 closure、102 份计划外部产物、空 base、策略或窗口中任一项变化都会使
四份签名同时失效。
...
closure 又显式引用 campaign/request/topology、102 份外部产物和 preapproval outputs，因此
