...
6. 当前质量门：backend `1192 passed, 19 skipped, 3 warnings`；真实开发 MySQL head=`0062`、`alembic check` clean；frontend `56 passed`、lint 0 warning 且 production build 通过；Ruff clean；mypy `81≤82`；OpenAPI v2 snapshot drift clean；Langfuse v4 compatibility adapter 保持解耦；生产静态基线 23/23 PASS。
