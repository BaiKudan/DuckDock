...
6. 当前质量门：backend `1253 passed, 19 skipped, 3 warnings`，核心 coverage `81.04%`；GA/生产运维聚焦 `252 passed`；真实开发 MySQL head=`0062`、`alembic check` clean；frontend `56 passed`、Playwright 3/3、lint 0 warning 且 production build 通过；Python/npm audit 与 Ruff clean；mypy `81≤82`；OpenAPI v2 snapshot drift clean；Langfuse v4 compatibility gate 60 项通过并保持适配器解耦；生产静态基线 36/36 PASS；一键 integrated preflight 73/73 PASS 且保留 6 项外部 PENDING。
