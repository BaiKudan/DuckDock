...
|---|---|---|
| PGA-01 | Production has a bundled TLS 1.2/1.3 edge; only 443 is public; HTTP/object/admin/monitoring ports are loopback or internal. | `docker-compose.prod-tls.yml`; production baseline report; live target TLS probe. |
...
| PGA-06 | A 900-second-or-longer capacity gate sustains the declared floor, materializes at least 50,000 runs, and proves post-growth timeline p95 through the real target HTTPS endpoint. | `duckdock-target-capacity-gate-v1` JSON. |
