...
| PGA-14 | During the gap between campaign preparation and all-or-nothing closure, release operators can produce immutable, non-authorizing progress checkpoints. Each inspection independently regenerates the campaign and topology, classifies all 66 planned external artifacts and 11 external phases, rejects symlinks, non-files, empty/oversized inputs, invalid JSON, private-key markers, unplanned references and declared-digest mismatches, distinguishes pre-window/active/expired and partial/invalid/ready/closed states, and gives the exact next phase or closure command. `ARTIFACTS_READY` and `READY_FOR_CLOSURE_ATTEMPT` explicitly do not assert evidence PASS or authorize mutation; only the original closure and full evaluator can advance to preapproval. | `duckdock-ga-execution-campaign-progress-v1`, immutable out-of-evidence-root checkpoints and `inspect_ga_execution_campaign.py --require-ready-for-closure`. |
